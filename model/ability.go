...
func getChannelQuery(group string, model string, retry int, tags []string) (*gorm.DB, error) {
	...
	*/
	if retry != 0 {
		...
		if err != nil {
			...
			channelQuery = DB.Where(commonGroupCol+" = ? and model = ? and enabled = ? and priority = ? and (tag ='' "+tagsWhere+")", group, model, true, priority)
...
func GetChannel(group string, model string, retry int, tags []string, path string) (*Channel, error) {
	...
	channelQuery, err := getChannelQuery(group, model, retry, tags)
	if err != nil {
		return nil, err
	}
