...
type responsesWSRouteBodyKey struct{}

// ResponsesWSRouteBody returns the normalized response.create request body
// used by the ordinary channel distributor. Invalid/control events do not
// carry one and remain the request worker's responsibility.
func ResponsesWSRouteBody(request *http.Request) ([]byte, bool) {
	body, ok := request.Context().Value(responsesWSRouteBodyKey{}).([]byte)
	return body, ok
}
...
func (s *responsesWSSession) runRequest(state *responsesWSCallState, message []byte, requestID string) {
	...
	create, id, normalizeErr := normalizeResponsesWSCreateEvent(message)
	eventID = id
	requestContext := s.ctx
	requestBody := message
	if normalizeErr == nil {
		requestContext = context.WithValue(requestContext, responsesWSRouteBodyKey{}, create.Body)
		requestBody = create.Body
	}
	request := s.request.Clone(requestContext)
	...
	apiErr = s.runner(request, requestID, func(c *gin.Context) *types.NewAPIError {
		if normalizeErr != nil {
			return newResponsesWSInvalidRequestError(normalizeErr)
		}
...
func (s *responsesWSSession) runCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	...
	if s.lockedChannelID != 0 {
		...
		for ; retry.GetRetry() <= retryPlan.MaxRetries; retry.IncreaseRetry() {
			...
			if apiErr != nil {
				if len(retryPlan.ChannelIds) > 0 && retry.GetRetry() < retryPlan.MaxRetries {
					continue
				}
				return apiErr
			}
...
func selectResponsesWSChannel(c *gin.Context, modelName string, retryParam *service.RetryParam) (*appmodel.Channel, *types.NewAPIError) {
	...
	if pin, found, overridden := constraints.ResolvedPin(); found {
		...
		alreadyPrepared := retryParam.GetRetry() == 0 && common.GetContextKeyInt(c, appconstant.ContextKeyChannelId) == channel.Id
		if !alreadyPrepared {
	...
	if len(retryParam.ChannelIds) == 0 && retryParam.GetRetry() == 0 {
		if selectedChannelID := common.GetContextKeyInt(c, appconstant.ContextKeyChannelId); selectedChannelID > 0 {
			selected, err := appmodel.CacheGetChannel(selectedChannelID)
			selectedUsable := false
			if err == nil && selected != nil && selected.Status == common.ChannelStatusEnabled {
				selectedUsable, _ = appmodel.ChannelSatisfiesFilters(selected, modelName, constraints.Filters)
			}
			if selectedUsable {
				return selected, nil
			}
