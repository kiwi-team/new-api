...
// omniPriceRatios 返回时长与清晰度对应的计费倍率，由 EstimateBilling 交给定价流程。
func omniPriceRatios(req *relaycommon.TaskSubmitReq) map[string]float64 {
	ratios := map[string]float64{
		"seconds": float64(resolveOmniSeconds(req)),
	}
	...
	if ratio, ok := omniResolutionRatios[resolution]; ok && ratio != 1 {
		ratios[fmt.Sprintf("resolution-%s", resolution)] = ratio
	}
	return ratios
}
