...
// SecondsRatio reads the billable video duration from the stored task request and
// returns it as an OtherRatios entry for per-second pricing. defaultSeconds is used
// when the client supplied neither `duration` nor `seconds`.
//
// The result is capped at relaycommon.MaxTaskDurationSeconds: the duration becomes a
// billing multiplier, so an unbounded value would let a single request overflow the
// quota calculation. ValidateBasicTaskRequest already rejects out-of-range durations
// with a 400; this cap is the second line of defence for adaptors reached by other paths.
func SecondsRatio(c *gin.Context, defaultSeconds int) map[string]float64 {
