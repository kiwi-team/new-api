...
func TestConsumeResponsesQuotaRecordsCapturedRequestAndResponse(t *testing.T) {
	...
	const requestBody = `{"model":"gpt-test","input":"hello"}`
	const responseBody = `{"id":"resp_test","model":"gpt-test","output":[]}`
	...
	ConsumeResponsesQuota(ctx, info, &dto.Usage{}, requestBody, responseBody)

	var stored model.Log
	require.NoError(t, database.Take(&stored).Error)
	assert.Equal(t, requestBody, stored.Request)
	assert.Equal(t, responseBody, stored.Response)
}
