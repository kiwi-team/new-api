...
func TestSelectResponsesWSChannelHonorsExplicitRouteOrder(t *testing.T) {
	database := setupRelayChannelDB(t)
	first := &model.Channel{Name: "first", Key: "sk-first", Status: common.ChannelStatusEnabled, Type: constant.ChannelTypeOpenAI}
	second := &model.Channel{Name: "second", Key: "sk-second", Status: common.ChannelStatusEnabled, Type: constant.ChannelTypeOpenAI}
	first.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
	second.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
	...
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	retry := 1
	channel, apiErr := selectResponsesWSChannel(c, "ws-model", &service.RetryParam{
		Ctx:        c,
		ModelName:  "ws-model",
		TokenGroup: "default",
		Retry:      &retry,
