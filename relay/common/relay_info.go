...
type RelayInfo struct {
	...
	// ChannelAttemptStartTime is set while the selected channel is in flight so
	// consume logs written inside the relay handler can include its elapsed time.
	...
	// WebSocket responses bypass the HTTP writer; cache their channel mapper so
	// high-frequency realtime frames do not reparse the mapping each time.
	modelOutputMappingRaw    string
...
func (info *RelayInfo) InitChannelMeta(c *gin.Context) {
	...
	if channelType == constant.ChannelTypeVolcEngine && info.RelayMode == relayconstant.RelayModeRealtime {
		channelMeta.ApiType = constant.APITypeVolcEngineRealtime
	}
