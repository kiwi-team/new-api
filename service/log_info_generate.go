...
func appendSettlementRatio(other *model.LogOther, gi hosttypes.GroupRatioInfo) {
	sr := gi.SettlementDiscount
	if sr <= 0 || sr == 1 || other == nil {
		return
	}
	other.SetPublic("settlement_ratio", sr)
	if gr, ok := other.Snapshot()["group_ratio"].(float64); ok && sr != 0 {
		// 还原折前分组倍率并做 6 位小数规整，避免浮点误差产生 0.6999999 之类的展示
		other.SetPublic("group_ratio", math.Round(gr/sr*1e6)/1e6)
	}
}
...
func AppendRelayLogAdminInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	channelTimes, _ := common.GetContextKeyType[[]int64](ctx, constant.ContextKeyUseChannelTime)
	if relayInfo != nil && !relayInfo.ChannelAttemptStartTime.IsZero() && len(channelTimes) < len(ctx.GetStringSlice("use_channel")) {
		channelTimes = append(slices.Clone(channelTimes), max(time.Since(relayInfo.ChannelAttemptStartTime).Milliseconds(), 1))
	}
	if len(channelTimes) > 0 {
		other.SetAdmin("use_channel_time", channelTimes)
	}
	if relayInfo != nil {
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.PriceData.GroupRatioInfo.UserGroupDiscount > 0 || relayInfo.PriceData.GroupRatioInfo.UserModelExtraDiscount > 0 {
		other.SetPublic("original_group_ratio", relayInfo.PriceData.GroupRatioInfo.OriginalGroupRatio)
	}
	if relayInfo.PriceData.GroupRatioInfo.UserGroupDiscount > 0 {
		other.SetPublic("user_group_discount", relayInfo.PriceData.GroupRatioInfo.UserGroupDiscount)
	}
	if relayInfo.PriceData.GroupRatioInfo.UserModelExtraDiscount > 0 {
		other.SetPublic("user_model_extra_discount", relayInfo.PriceData.GroupRatioInfo.UserModelExtraDiscount)
	}
...
func GenerateWssOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage, modelRatio, groupRatio, completionRatio, audioRatio, audioCompletionRatio, videoRatio, modelPrice, userGroupRatio float64) *model.LogOther {
	...
	info.SetPublic("video_ratio", videoRatio)
	return info
}
...
func GenerateMjOtherInfo(relayInfo *relaycommon.RelayInfo, priceData hosttypes.PriceData) *model.LogOther {
	...
	if priceData.GroupRatioInfo.UserGroupDiscount > 0 {
		other.SetPublic("user_group_discount", priceData.GroupRatioInfo.UserGroupDiscount)
	}
	if priceData.GroupRatioInfo.UserModelExtraDiscount > 0 {
		other.SetPublic("user_model_extra_discount", priceData.GroupRatioInfo.UserModelExtraDiscount)
	}
