...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string, requestStr string, responseStr string) {
	originUsage := usage
	...
	if bodyReadMs := common.GetContextKeyInt(ctx, constant.ContextKeyRequestBodyReadTime); bodyReadMs > 0 {
		other.SetPublic("body_read_time_ms", bodyReadMs)
	}

	// TTS 按输入字符数计费，仅覆盖日志展示的额度与内容，不改变上面已结算的扣费。
	logQuota := summary.Quota
	if ttsCount := common.GetContextKeyInt(ctx, constant.ContextKeyTTSCount); ttsCount > 0 {
		ttsRatio := ratio_setting.GetTTSRatio(relayInfo.UpstreamModelName)
		logQuota = common.QuotaFromFloat(float64(ttsCount) / 1000 * ttsRatio)
		logContent = fmt.Sprintf("ttsRatio:%.2f，TTS 输入字符数:%d，TTS语音计费: %s", ttsRatio, ttsCount, logger.FormatQuota(logQuota))
	}

	// Track project consumption and get project name for logging
	projectName, planId, _ := TrackProjectConsumption(ctx, logQuota)

	var usageStr string
	if billingUsage != nil {
		if usageBytes, err := common.Marshal(billingUsage); err == nil {
	...
	model.RecordConsumeLog(ctx, relayInfo.UserId, model.RecordConsumeLogParams{
		...
		ClaudeCacheCreation5mTokens: summary.CacheCreationTokens5m,
		ClaudeCacheCreation1hTokens: summary.CacheCreationTokens1h,
		ModelName:                   logModel,
