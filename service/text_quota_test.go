...
func TestCalculateTextQuotaSummaryNormalizesOpenAIResponsesBillingUsageDetails(t *testing.T) {
	...
	// Consume/quota logs use the normalized uncached prompt count. The original
	// total remains available as InputTokens on the canonical usage.
...
func TestAppendRelayLogAdminInfoWritesChannelAttemptTimes(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx, _ := gin.CreateTestContext(httptest.NewRecorder())
	ctx.Set("use_channel", []string{"1578", "1577"})
	common.SetContextKey(ctx, constant.ContextKeyUseChannelTime, []int64{343})

	other := model.NewLogOther()
	AppendRelayLogAdminInfo(ctx, &relaycommon.RelayInfo{
		ChannelAttemptStartTime: time.Now().Add(-1200 * time.Millisecond),
	}, other)

	adminInfo, ok := other.Snapshot()["admin_info"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, []string{"1578", "1577"}, adminInfo["use_channel"])
	times, ok := adminInfo["use_channel_time"].([]int64)
	require.True(t, ok)
	require.Len(t, times, 2)
	require.Equal(t, int64(343), times[0])
	require.GreaterOrEqual(t, times[1], int64(1200))
}
