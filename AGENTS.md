...
### Main merge regression protection (mandatory)

- Before merging or resolving conflicts from `main`, read `MERGE_CONFLICT_RESOLUTION.md`, especially the section “Future main merge non-regression contract”. Its listed behaviors are intentional project capabilities, not obsolete merge leftovers.
- Do not resolve an affected file by taking the whole `main` side when that would delete or bypass a protected behavior. Port the behavior onto the new architecture and preserve its authorization boundary, persistence contract, and frontend entry point.
- A feature being absent from `main` is not evidence that it should be removed here. Removal or an incompatible behavior change requires explicit user approval and a corresponding update to the non-regression record.
- After an affected merge, run the relevant anchor tests listed in that record. Compilation alone is insufficient because the prior regressions compiled successfully while silently dropping UI controls, routing rules, or log payloads.
- Record newly discovered post-merge regressions and their regression tests in `MERGE_CONFLICT_RESOLUTION.md` so the next merge treats them as protected behavior.
