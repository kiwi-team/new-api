...
var responsesWSRequestEngine = sync.OnceValue(func() *gin.Engine {
	engine := gin.New()
	distribute := middleware.Distribute()
	...
	engine.POST("/v1/responses", func(c *gin.Context) {
		...
		if _, ok := relay.ResponsesWSRouteBody(c.Request); !ok {
			c.Next()
			return
		}
		distribute(c)
...
func newResponsesWSRequestRunner(c *gin.Context) relay.ResponsesWSRequestRunner {
	...
	return func(request *http.Request, requestID string, handle func(*gin.Context) *types.NewAPIError) *types.NewAPIError {
		...
		if routeBody, ok := relay.ResponsesWSRouteBody(request); ok {
			request.Body = io.NopCloser(bytes.NewReader(routeBody))
			request.ContentLength = int64(len(routeBody))
		}
