...
func channelHasSensitiveChanges(channel *PatchChannel, origin *model.Channel, requestData map[string]any) bool {
	...
	// ratio is the per-channel price multiplier: editing it changes what users
	// are billed for every request routed through the channel.
	if _, ok := requestData["ratio"]; ok && !equalFloat64Ptr(channel.Ratio, origin.Ratio) {
		return true
	}
