...
func filterExplicitChannelCandidates(channelIds []int, modelName string, tags []string, constraints *taskdto.ChannelConstraints) []int {
	...
	for _, channelId := range channelIds {
		...
		if ok, _ := model.ChannelSatisfiesFilters(candidate, modelName, constraints.Filters); !ok {
			continue
		}
...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		channelRulesMapAny, hasChannelRules := c.Get("token_channel_rules")
		channelRulesMap := make(map[string]taskdto.ChannelRulesItem)
		if hasChannelRules {
			channelRulesMap, _ = channelRulesMapAny.(map[string]taskdto.ChannelRulesItem)
		}
		var channelRules *taskdto.ChannelRulesItem
		for pattern, rule := range channelRulesMap {
		...
		if channelRules == nil {
			for pattern, rule := range channelRulesMap {
				if common.RegMatch(pattern, modelName) {
					channelRules = &rule
					break
				}
			}
	...
	if err == nil {
		for _, item := range specialChannels {
			if item.ModelName == modelName {
				//ids = item.ChannelIds
				for _, arr := range item.ChannelIds {
					common.ShuffleSlice(arr)
					ids = append(ids, arr...)
				}
