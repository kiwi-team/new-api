...
const migratedProviders = [
  ...
  { type: 53, label: 'Jimeng', key: 'jimeng' },
  { type: 55, label: 'Vidu', key: 'vidu' },
  { type: 57, label: 'DoubaoVideo', key: 'doubao' },
  { type: 58, label: 'Sora', key: 'sora' },
]
...
test('creation hides legacy Zhipu from categories and numeric search while GLM remains selectable', async () => {
  ...
  await user.click(
    screen.getByRole('option', { name: 'Zhipu V4 Built-in #26' })
  )
...
test('Zhipu V4 uses the renamed Chinese label and remains searchable after changing language', async () => {
  ...
  await user.type(screen.getByRole('combobox'), 'V4')
  expect(
    screen.getByRole('option', { name: 'Zhipu V4 Built-in #26' })
  ).toBeVisible()
  await act(() => language.changeLanguage('zh'))
  expect(screen.getByRole('option', { name: '智谱 V4 内置 #26' })).toBeVisible()
})
...
test('plugin extensions keep built-in and independent plugin choices distinct in every category', async () => {
  ...
  expect(
    screen.queryByRole('option', { name: 'Sora Built-in #58' })
  ).not.toBeInTheDocument()
...
test('extension associations follow declared types and exclude legacy task-only types', () => {
  render(
    <ChannelProviderPicker
      plugins={[
        { ...extensionPlugin, name: 'OpenAI', channelTypes: [24, 58, 61, 999] },
      ]}
      canBindPlugin
      loading={false}
      failed={false}
...
test.each([
  ...
  async (state) => {
    ...
    await user.click(screen.getByRole('option', { name: 'Sora Built-in #58' }))
    expect(select).toHaveBeenCalledWith({ kind: 'builtin', type: 58 })
  }
)
...
test.each([true, false])(
  ...
  async (canBindPlugin) => {
    ...
    for (const category of ['All', 'Built-in', 'Gateways']) {
      ...
      expect(
        screen.getByRole('option', { name: 'New API Built-in #76' })
      ).toBeVisible()
      expect(
        screen.getByRole('option', { name: 'Sub2API Built-in #75' })
      ).toBeVisible()
    }
    expect(screen.getAllByRole('option')).toHaveLength(2)
    await user.click(
    ...
    expect(select).toHaveBeenLastCalledWith({ kind: 'builtin', type: 76 })
    const search = screen.getByRole('combobox')
    ...
    expect(select).toHaveBeenLastCalledWith({ kind: 'builtin', type: 75 })
    await user.click(screen.getByRole('tab', { name: 'Built-in' }))
...
test('custom providers have their own tab and keep the search when switching categories', async () => {
  ...
  expect(
    within(panel).getByRole('option', { name: 'Advanced Custom Built-in #74' })
  ).toBeVisible()
  expect(
  ...
  expect(
    screen.queryByRole('option', { name: 'Advanced Custom Built-in #74' })
  ).not.toBeInTheDocument()
  expect(
    screen.queryByRole('option', { name: 'Custom Built-in #8' })
  ).not.toBeInTheDocument()
  await user.type(screen.getByRole('combobox'), '74')
  expect(screen.queryByRole('option')).not.toBeInTheDocument()
  await user.click(within(tabs).getByRole('tab', { name: 'Custom' }))
  expect(screen.getByRole('combobox')).toHaveValue('74')
  expect(screen.getAllByRole('option')).toHaveLength(1)
  expect(
    screen.getByRole('option', { name: 'Advanced Custom Built-in #74' })
  ).toBeVisible()
})
...
test('deprecated and flexible integration badges preserve provider selection and expose the full explanation', async () => {
  ...
  const advanced = screen.getByRole('option', {
    name: 'Advanced Custom Built-in #74',
  })
  ...
  expect(select).toHaveBeenNthCalledWith(2, { kind: 'builtin', type: 74 })
