...
const soraPlugin: TaskPluginOption = {
  ...
  channelTypes: [CHANNEL_TYPE_SORA, 1],
  models: ['sora-2'],
}
...
test('restores request path, response model, and Vertex bucket settings in the channel editor', async () => {
  editingChannel = channelSchema.parse({
    ...editingChannel,
    type: 41,
    other: 'us-central1',
    setting: JSON.stringify({
      google_file_bucket: 'vertex-media',
      model_output_mapping: '{"upstream-model":"public-model"}',
      path_whitelist: ['/v1/messages', '/v1/chat'],
      path_blacklist: ['/v1/embeddings'],
    }),
  })
  const put = vi
    .spyOn(api, 'put')
  ...
  expect(await screen.findByLabelText('Google file bucket')).toHaveValue(
    'vertex-media'
  )
  await user.click(screen.getByRole('tab', { name: /Request & Response/ }))
  expect(screen.getByLabelText('Model output mapping')).toHaveValue(
    '{"upstream-model":"public-model"}'
  )
  expect(screen.getByLabelText('Path whitelist')).toHaveValue(
    '/v1/messages\n/v1/chat'
  )
  expect(screen.getByLabelText('Path blacklist')).toHaveValue('/v1/embeddings')
...
test.each(['create', 'edit'])(
  ...
  async (mode) => {
    pluginOptions = [
      {
        ...soraPlugin,
        channelTypes: [1, CHANNEL_TYPE_SORA],
...
test('retrying extension metadata preserves the editable built-in draft and selected models', async () => {
  pluginOptions = [{ ...soraPlugin, channelTypes: [1, CHANNEL_TYPE_SORA] }]
  const originalGet = vi.mocked(api.get).getMockImplementation()
  let fail = true
  vi.mocked(api.get).mockImplementation(async (url, config) => {
    if (url === '/api/task_plugin_options' && fail) {
...
test('loading a replacement plugin preserves an already selected legacy creation draft', async () => {
  ...
  const legacy = screen.getByRole('option', { name: 'Sora Built-in #58' })
...
test('editing legacy channels retains the full provider list and saves the original type', async () => {
  editingChannel = { ...editingChannel, type: CHANNEL_TYPE_SORA }
  pluginOptions = [
    soraPlugin,
    {
      key: 'doubao',
  ...
  expect(
    screen.getByRole('option', { name: 'DoubaoVideo Built-in #57' })
  ).toBeVisible()
  const legacy = screen.getByRole('option', { name: 'Sora Built-in #58' })
  expect(legacy).toHaveAttribute('aria-current', 'true')
  await user.click(legacy)
