...
async function renderKeysPage(
  ...
  useAuthStore.getState().auth.setUser({
    id: 1,
    username: role === ROLE.SUPER_ADMIN ? 'root' : 'alice',
    role,
  })
  vi.mocked(api.get).mockImplementation(async (url) => {
    if (url === '/api/user/') {
      return {
        data: {
          success: true,
          data: { items: [{ id: 42, username: 'target-user' }] },
        },
      }
    }
...
it('lets root filter the key list by user and scopes the backend request', async () => {
  await renderKeysPage(1, {}, ROLE.SUPER_ADMIN)
  const user = userEvent.setup()
  const filter = await screen.findByRole('combobox', { name: 'Filter by user' })

  await user.click(filter)
  await user.click(
    await screen.findByRole('option', { name: 'target-user (ID: 42)' })
  )

  await waitFor(() =>
    expect(api.get).toHaveBeenCalledWith(expect.stringContaining('user_id=42'))
  )
})
