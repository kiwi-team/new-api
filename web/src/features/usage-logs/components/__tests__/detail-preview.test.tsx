...
function renderPreview(other: LogOtherData, isAdmin = true, isRoot = false) {
  render(
    <I18nextProvider i18n={i18n}>
      <QueryClientProvider client={client}>
        <DetailPreview other={other} isAdmin={isAdmin} isRoot={isRoot} />
      </QueryClientProvider>
    </I18nextProvider>
  )
...
test.each([
  ...
  { name: 'admin', role: ROLE.ADMIN, visible: false },
  { name: 'user', role: ROLE.USER, visible: false },
])(
  '$name raw payload entry visibility is root-only',
  async ({ role, visible }) => {
    useAuthStore.getState().auth.setUser({ id: 1, username: 'test', role })
    const preview = renderPreview({}, role >= ROLE.ADMIN, false)
    fireEvent.click(preview)
    const dialog = within(await screen.findByRole('dialog'))
    const entry = dialog.queryByRole('button', { name: 'View raw payload' })
