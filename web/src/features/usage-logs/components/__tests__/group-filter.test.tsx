...
it('restores all correlation filters from the URL and preserves them on search', async () => {
  const router = await renderFilter(
    '/usage-logs/common?clientUserId=client-42&mtSessionId=mt-7&traceId=trace-8&trajId=traj-9&sessionId=session-10'
  )
  await userEvent.click(screen.getByRole('button', { name: /^Expand/ }))

  expect(screen.getByRole('combobox', { name: 'Client UID' })).toHaveValue(
    'client-42'
  )
  expect(screen.getByPlaceholderText('MT Session ID')).toHaveValue('mt-7')
  expect(screen.getByPlaceholderText('Trace ID')).toHaveValue('trace-8')
  expect(screen.getByPlaceholderText('Traj ID')).toHaveValue('traj-9')
  expect(screen.getByPlaceholderText('Session ID')).toHaveValue('session-10')
