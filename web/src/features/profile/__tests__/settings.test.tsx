...
describe('user settings saves across profile and security', () => {
  ...
  it('shows and saves the account model limits from the profile', async () => {
    const modelLimitSettings = {
      ...settings,
    ...
    vi.spyOn(api, 'get').mockImplementation(async (url) => {
      if (url === '/api/user/models') {
        return {
          data: { success: true, data: ['gpt-4o', 'claude-3-5-sonnet'] },
        }
      }
      if (url === '/api/settlement/config/self') {
        return {
          data: {
            success: true,
            data: [{ model_name: 'gpt-4o' }, { model_name: 'gemini-2.5-pro' }],
          },
        }
      }
      return {
        data: {
          ...
          data: {
            ...profile,
            setting: JSON.stringify(modelLimitSettings),
          },
        },
      }
    })
    ...
    expect(
      screen.getByRole('switch', { name: 'Enable model limits' })
    ).toBeChecked()
    expect(screen.getByText('gpt-4o')).toBeInTheDocument()

    fireEvent.click(
      screen.getByRole('button', {
        name: 'Limit to models with a settlement price',
      })
    )
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(put).toHaveBeenCalledWith('/api/user/setting', {
        ...modelLimitSettings,
        model_limits: ['gpt-4o', 'gemini-2.5-pro'],
      })
    )
    queryClient.clear()
  })
