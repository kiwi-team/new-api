	...
	for _, contentItemAny := range arrayContent {
		...
		case ContentTypeVideoUrl:
			if videoUrl, ok := contentItem["video_url"].(string); ok {
				...
				if videoURL, ok := videoMap["url"].(string); ok {
					contentList = append(contentList, MediaContent{
						Type:     ContentTypeVideoUrl,
		...
		case ContentTypeAudioUrl:
			if audioURL, ok := contentItem["audio_url"].(string); ok {
				contentList = append(contentList, MediaContent{
					...
					AudioUrl: &MessageAudioUrl{Url: audioURL},
				})
			} else if audioMap, ok := contentItem["audio_url"].(map[string]any); ok {
				if audioURL, ok := audioMap["url"].(string); ok {
					contentList = append(contentList, MediaContent{
						Type:     ContentTypeAudioUrl,
						AudioUrl: &MessageAudioUrl{Url: audioURL},
					})
		...
		if len(contentList) > lenBefore {
			if cacheControl, ok := contentItem["cache_control"]; ok && cacheControl != nil {
...
// GetEnableThinking 读取 enable_thinking 的布尔语义。
// ok 为 false 表示客户端未传该字段，或传的不是布尔值（例如对象形式），
// 调用方应当保持原样透传而不是当成 false。
func (r *GeneralOpenAIRequest) GetEnableThinking() (value bool, ok bool) {
	if len(r.EnableThinking) == 0 {
		return false, false
	}
	if err := json.Unmarshal(r.EnableThinking, &value); err != nil {
		return false, false
	}
	return value, true
}

// SetEnableThinking 以布尔值覆盖 enable_thinking。
func (r *GeneralOpenAIRequest) SetEnableThinking(value bool) {
	if value {
		r.EnableThinking = json.RawMessage("true")
		return
	}
	r.EnableThinking = json.RawMessage("false")
}
